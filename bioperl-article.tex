...
% $Id: bioperl-article.tex,v 1.18 2002-04-07 20:47:05 jason Exp $
...
Access project.  We conclude by outling how the open source nature of
...
particular lab or institution and is not written in a generic manner
so that it can be used by others.  The Bioperl toolkit brings together
...
start, end, strand, and access to annotation via tag/value pairs.  All
...
server, a local file, or a database server can all be drawn as an
...
OS X and Microsoft Windows operating systems.  Since the toolkit is
run through the Perl interpreter, specific issues are often due to
compatibility between different versions of Perl.  Descriptions of
operating-system specific problems and their solutions are available
...
Language (\url{http://www.bsml.org}) ad BGDP's
...
Databases \cite{embl-corba} in addition to and implementations are
...
In the future, Bioperl will continue to evolve to address
more domains of bioinformatics.  We plan to create objects to
...
multi-species comparisons will build Perl modules to manage the
alignment and syntenic information.  We will create software layers
to interact with OBDA databases, develop a generic analysis pipeline
system to provide automated analysis components, and expand the supported
