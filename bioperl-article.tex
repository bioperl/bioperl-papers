...
% $Id: bioperl-article.tex,v 1.14 2002-04-05 17:19:02 jason Exp $
...
\usepackage { listings,url,cite,layouts,graphics,keyval,fullpage,doublespace }
...
that it can be used by others.  The Bioperl toolkit is a collection of
Perl modules containing generalized routines specific to biological
and life science information which are easily reused.  A primary
motivation behind writing the toolkit is the authors' desire to focus
our energies on one solution whose components could be shared rather
than duplicating effort.  In our minds, once a solution for parsing
and interpreting sequence from EMBL and Genbank format sequence files
is written, no one else should have to worry about writing their own.
...
as well.  We have found that like the human genome project, openess
has reduced the time needed for solutions and new tools to reach the
...
from a file for users who do not want to understand the entire module
hierarchy.
...
that are useful for a variety of tasks.  These include reading and
...
This code retrieve an mRNA sequences from the EBI EMBL
...
\end{lstlisting}
\end{scriptsize}
\caption{BLAST parsing. A BLAST report is parsed and only hits meeting
...
more issues in bioinformatics.  We have plans to create objects to
